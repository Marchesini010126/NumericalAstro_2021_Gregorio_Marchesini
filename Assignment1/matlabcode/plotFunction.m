...
addpath('/Users/gregorio/Desktop/matlabUtilities')
...
kep_labels = {'\Delta a [m]','\Delta e','i [rad]','\Delta \omega [rad]','\Delta \Omega [rad]','\Delta \theta[rad]'};
...
fig_task7_perturbed=figure('Position',[50,50,900,600],'visible',visible);
...
fig_task7_unperturbed=figure('Position',[50,50,900,600],'visible','on');
...
semimajor_axis     = KeplerianState_perturbed(:,2)';
...
plot(time_days,dn_sum-mean(dn_sum));
hold on
plot(time_days,delta_mean_anomaly- mean(delta_mean_anomaly));
%hol on
%plot(time_days,dn_sum.*ones(1,length(time_days)) );
xlabel('days')
ylabel('\Delta n [1/s]')
